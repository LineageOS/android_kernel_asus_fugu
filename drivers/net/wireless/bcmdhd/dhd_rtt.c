...
dhd_rtt_start(dhd_pub_t *dhd) {
	...
	err = dhd_iovar(dhd, 0, "proxd", (char *)&proxd_iovar,
		sizeof(proxd_iovar), NULL, 0, TRUE);
	...
	err = dhd_iovar(dhd, 0, "proxd_tune", (char *)&proxd_tune,
		sizeof(proxd_tune), NULL, 0, TRUE);
	if (err < 0) {
	...
	err = dhd_iovar(dhd, 0, "proxd_params", (char *)&proxd_params,
		sizeof(proxd_params), NULL, 0, TRUE);
	if (err < 0) {
